...
29. `selectColumns`, `dropColumns`, `renameColumn`, `cloneColumn`, `castColumn`, `formula`, and `textLength` accept only public column references shaped as `{id, name}`. The runtime binds each reference against the exact input schema and lineage to a private `{id, name, position}` value before adapter dispatch; persisted steps and protocol metadata never expose that position. Unknown, stale, mismatched, or malformed identities, duplicate list selections or input-lineage IDs, output-name collisions, and private row-identity namespace collisions fail closed. No string/name fallback is permitted, every transformed result must retain one visible column, Pandas live execution plus generated code must address these operations positionally, and an edited latest step must retain its original step ID while deriving globally unique new-output IDs as `c:step:<stepId>:<created-output-ordinal>`. State-dependent reuse of the replaced result's identities is forbidden so replay publishes the same edited lineage.
